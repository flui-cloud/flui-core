...
function buildExample(framework?: string): string {
  ...
  return [
    ...
    'build:',
    '  strategy: dockerfile            # build the repo Dockerfile',
    ...
    '      value: production',
    '  scaling:',
    '    min: 1                        # replicas it always runs',
    '    max: 3                        # more replicas are added when CPU passes 80% of the request',
    '                                  # (CPU, not memory: memory rarely falls after a peak)',
    '  # domain:',
    ...
    '  # resources.profile · env[].valueFrom · env[].secret',
