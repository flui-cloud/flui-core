...
export default class IamGroupRemoveMember extends Command {
  static readonly description =
    'Remove a member (by email) from a Flui-local group.';
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(IamGroupRemoveMember);
    ...
    let group: IamGroup;
    try {
      group = await api.delete<IamGroup>(
      ...
      this.error(`Failed to remove member: ${(error as Error).message}`, {
        exit: 1,
      });
    }
    ...
    console.log(
      `\n  ${chalk.green('✓')} Removed ${chalk.bold(args.email)} from ${chalk.bold(args.name)} ${chalk.dim(`(${group.members.length} members)`)}\n`,
    );
  }
}
