...
export default class MigrateDbCutover extends Command {
  static readonly description =
    'Fire the cutover for a parked (manual) DB migration';
  static readonly args = {
    id: Args.string({ required: true, description: 'Migration id' }),
  };

  async run(): Promise<void> {
    const { args } = await this.parse(MigrateDbCutover);
    printContextBanner();
    const spinner = ora(`Cutting over ${args.id}...`).start();
    try {
      const mig = await MigrationClient.fromConfig().cutoverDbMigration(
        args.id,
      );
