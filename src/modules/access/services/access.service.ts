...
export class AccessService {
  ...
  async addSSHKey(
    ...
    const createdKey = await this.repository.createKey(
      input,
      {
        ...keyPair,
        privateKey: keyPath,
        sealedPrivateKey: this.keyStorage.encryptKeyToString(
          keyPair.privateKey,
        ),
      },
      input.tags,
  ...
  async createSSHKey(input: {
    ...
    const createdKey = await this.repository.createKey(
      ...
      },
      {
        ...
        sealedPrivateKey: this.keyStorage.encryptKeyToString(input.private_key),
      },
  ...
  async getPrivateKey(userId: string, keyId: string): Promise<string> {
    ...
    return this.privateKeyOf(key);
  }

  /**
   * The private half of a key: from its record, where it is kept sealed; from
   * the key file only for a record written before that, which is then copied
   * into the record so the next restart of the API cannot lose it.
   */
  private async privateKeyOf(key: SSHKey): Promise<string> {
    if (key.sealedPrivateKey) {
      return this.keyStorage.decryptKeyFromString(key.sealedPrivateKey);
    }
    const privateKey = await this.keyStorage.retrievePrivateKey(key.keyPath);
    await this.repository.updateKey(key.id, {
      sealedPrivateKey: this.keyStorage.encryptKeyToString(privateKey),
    });
    return privateKey;
  }
  ...
  async getBootstrapKeyMaterialForCluster(clusterId: string): Promise<{
    ...
    if (
      !keyEntity.sealedPrivateKey &&
      !this.keyStorage.isStoredHere(keyEntity.keyPath)
    ) {
      this.logger.warn(
    ...
    try {
      privateKey = await this.privateKeyOf(keyEntity);
    } catch (err) {
  ...
  async getBootstrapPrivateKeyForCluster(clusterId: string): Promise<string> {
    ...
    if (cluster?.bootstrapKeyId) {
      ...
      if (keyEntity) {
        return this.privateKeyOf(keyEntity);
      }
    }
    ...
    return this.privateKeyOf(latest);
  }
}
